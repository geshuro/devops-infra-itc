...
data "terraform_remote_state" "kubernetes-release" {
  backend = "s3"
  config = {
    profile        = var.BackendProfile
    bucket         = var.BackendS3
    key            = "terraform/release"
    region         = var.BackendRegion
    encrypt        = true
    dynamodb_table = var.BackendDynamoDB // Nombre de la tabla que almacena el estado de terraform.
  }
...
resource "aws_security_group_rule" "kubernetes-dev_to_fhir_http" {
  type              = "ingress"
  from_port         = 80
  to_port           = 80
  protocol          = local.protocol_tcp
  source_security_group_id  = data.terraform_remote_state.kubernetes-dev.outputs.server_security_group_id[0]
  security_group_id = aws_security_group.fhir_access.id
  description = "fhir http communication from Kubernetes Dev"
}

resource "aws_security_group_rule" "kubernetes-dev_to_fhir_https" {
  type              = "ingress"
  from_port         = 443
  to_port           = 443
  protocol          = local.protocol_tcp
  source_security_group_id  = data.terraform_remote_state.kubernetes-dev.outputs.server_security_group_id[0]
  security_group_id = aws_security_group.fhir_access.id
  description = "fhir https communication from Kubernetes Dev"
}

resource "aws_security_group_rule" "kubernetes-release_to_fhir_http" {
  type              = "ingress"
  from_port         = 80
  to_port           = 80
  protocol          = local.protocol_tcp
  source_security_group_id  = data.terraform_remote_state.kubernetes-release.outputs.server_security_group_id[0]
  security_group_id = aws_security_group.fhir_access.id
  description = "fhir http communication from Kubernetes Release"
}

resource "aws_security_group_rule" "kubernetes-release_to_fhir_https" {
  type              = "ingress"
  from_port         = 443
  to_port           = 443
  protocol          = local.protocol_tcp
  source_security_group_id  = data.terraform_remote_state.kubernetes-release.outputs.server_security_group_id[0]
  security_group_id = aws_security_group.fhir_access.id
  description = "fhir https communication from Kubernetes Release"
}