...
[ -f /home/ubuntu/config/aws-ec2-ssh.conf ] && . /home/ubuntu/config/aws-ec2-ssh.conf
...
if [ ${DONOTSYNC} -eq 1 ]
then
    log "Please configure aws-ec2-ssh by editing /home/ubuntu/config/aws-ec2-ssh.conf"
    exit 1
fi
...
function create_or_update_local_user() {
    ...
    # Check si existe en openvpn sino existe crear.
    if [[ -z "$(tail -n +2 /home/ubuntu/config/openvpn/easy-rsa/pki/index.txt | grep "^V" | cut -d '=' -f 2 | grep -w ${username})" ]]; then
   	/opt/openvpn/manage-openvpn.sh -a ${username}
    fi
    ...
    then
        ...
        SaveUserSudoFilePath="/home/ubuntu/config/sudoers.d/$SaveUserFileName"
...
function delete_local_user() {
    ...
    if [[ ! -z "$(tail -n +2 /home/ubuntu/config/openvpn/easy-rsa/pki/index.txt | grep "^V" | cut -d '=' -f 2 | grep -w ${1})" ]]; then
 	/opt/openvpn/manage-openvpn.sh -r "${1}"
    fi
