...
data "terraform_remote_state" "pro-networking" {
  backend = "s3"
  config = {
    profile        = var.AWSprofileBackend 
    bucket         = var.s3backend
    key            = var.prokeybackend
    region         = var.regionbackend
    encrypt        = true
    dynamodb_table = var.DynamoDBbackend // Nombre de la tabla que almacena el estado de terraform.
  }
...
# shared to pro for nat
resource "aws_route" "route-peering-nat-shared-pro" {
  route_table_id            = data.terraform_remote_state.shared-networking.outputs.route_table-public_bastion_subnet-ids[0]
  destination_cidr_block    = data.terraform_remote_state.pro-networking.outputs.vpc_cidr_block
  vpc_peering_connection_id = data.terraform_remote_state.pro-networking.outputs.vpc_peering_shared_id
}
...
# shared to pro for pub
resource "aws_route" "route-peering-pub-shared-pro" {
  route_table_id            = data.terraform_remote_state.shared-networking.outputs.route_table-public_subnet-ids[0]
  destination_cidr_block    = data.terraform_remote_state.pro-networking.outputs.vpc_cidr_block
  vpc_peering_connection_id = data.terraform_remote_state.pro-networking.outputs.vpc_peering_shared_id
}
...
# shared to pro for int
resource "aws_route" "route-peering-int-shared-pro" {
  route_table_id            = data.terraform_remote_state.shared-networking.outputs.route_table-private_internal_subnet-ids[0]
  destination_cidr_block    = data.terraform_remote_state.pro-networking.outputs.vpc_cidr_block
  vpc_peering_connection_id = data.terraform_remote_state.pro-networking.outputs.vpc_peering_shared_id
}
...
resource "aws_route" "route-peering-pri-za-shared-pro" {
  route_table_id            = data.terraform_remote_state.shared-networking.outputs.route_table-private_subnet-ids-za[0]
  destination_cidr_block    = data.terraform_remote_state.pro-networking.outputs.vpc_cidr_block
  vpc_peering_connection_id = data.terraform_remote_state.pro-networking.outputs.vpc_peering_shared_id
}
...
resource "aws_route" "route-peering-pri-zb-shared-pro" {
  route_table_id            = data.terraform_remote_state.shared-networking.outputs.route_table-private_subnet-ids-zb[0]
  destination_cidr_block    = data.terraform_remote_state.pro-networking.outputs.vpc_cidr_block
  vpc_peering_connection_id = data.terraform_remote_state.pro-networking.outputs.vpc_peering_shared_id
}
...
#Añadir VPC Pro
resource "aws_ec2_transit_gateway_vpc_attachment" "transit-gateway-attachment-pro" {
  subnet_ids         = data.terraform_remote_state.pro-networking.outputs.private_2_subnet-id
  transit_gateway_id = aws_ec2_transit_gateway.hub.id
  vpc_id             = data.terraform_remote_state.pro-networking.outputs.vpc_id
  transit_gateway_default_route_table_association = false
  transit_gateway_default_route_table_propagation = false
  tags = {
    Name         = "tgw-attach-pro"
...
resource "aws_ec2_transit_gateway_route_table_association" "transit-gateway-route-association-pro" {
  transit_gateway_attachment_id  = aws_ec2_transit_gateway_vpc_attachment.transit-gateway-attachment-pro.id
  transit_gateway_route_table_id = aws_ec2_transit_gateway_route_table.route-table-transit-gateway-hub.id
}